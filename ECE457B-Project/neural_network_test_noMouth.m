...
NODES = [48, 200, 4]; % total number of nodes (without bias in layer)
LAYERS = length(NODES);
...
load('Dataset.mat');
[totalInputs, allExemplars] = size(allLandmarks(1:48,:));
...
trainingMean = mean(allLandmarks(1:48,1:numTraining),2)*ones(1,length(allLandmarks(1:48,1:numTraining)));
trainingInputs = allLandmarks(1:48,1:numTraining) - trainingMean;

% calculate means across single axis and subtract them for test data
testMean = mean(allLandmarks(1:48,numTraining+1:end),2)*ones(1,numTest); % mean from test data
testInputs = allLandmarks(1:48,numTraining+1:end) - testMean;
...
while ((E_percent_train >= E_max && epoch < maxEpochs) && (E >= E_test))  % loop to check error
    E = 0;  % reset error
